...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".has-animation").forEach((element) => {
    try {
      ...
      if (!element) {
        console.warn("AnimateWP: Invalid element encountered, skipping animation");
        return;
      }
      ...
      const direction = element.getAttribute("data-animation-direction") || "from";
    ...
    if (autoPlay) {
      ...
      console.error("AnimateWP: Error creating animation for element:", element, error);
    }
  });
});
